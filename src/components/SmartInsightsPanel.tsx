  ...
  const insights: Insight[] = useMemo(() => {
    ...
    if (outliers.length > 0) {
      generatedInsights.push({
        ...
        description: `${outliers.length} cost categories exceed recommended thresholds`,
        trend: 'up'
      });
    }

    if (adjusted.profitPerKg > results.profitPerKg) {
      generatedInsights.push({
        type: 'success',
        title: 'Improved Margin',
        description: 'Current margin settings show improved profitability',
        value: adjusted.profitPerKg,
        trend: 'up'
      });
    }

    return generatedInsights;
  }, [outliers, adjusted, results]);
