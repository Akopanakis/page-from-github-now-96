...
export const validateFormData = (data: Partial<FormData>): ValidationResult => {
    ...
    data.workers.forEach((w, idx) => {
      if (
        ...
        (w.hourlyRate !== undefined &&
          (typeof w.hourlyRate !== "number" || Number.isNaN(w.hourlyRate))) ||
        (w.hours !== undefined &&
          (typeof w.hours !== "number" || Number.isNaN(w.hours)))
      ) {
  ...
  if (data.processingPhases && Array.isArray(data.processingPhases)) {
    data.processingPhases.forEach((p, idx) => {
      if (
        p != null &&
        ((p.wastePercentage !== undefined &&
          (typeof p.wastePercentage !== "number" ||
            Number.isNaN(p.wastePercentage))) ||
          (p.addedWeight !== undefined &&
            (typeof p.addedWeight !== "number" || Number.isNaN(p.addedWeight))))
      ) {
        console.debug(
          `Processing phase ${idx} has invalid data, will use defaults`,
        );
      }
    });
  }
